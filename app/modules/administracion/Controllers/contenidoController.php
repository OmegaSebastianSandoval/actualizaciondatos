...
class Administracion_contenidoController extends Administracion_mainController
{
	...
	public function indexAction()
	{
		...
		$contentpadre = $this->mainModel->getById($this->_getSanitizedParam("padre"));
		$externo = 0;
		if($contentpadre->contenido_enlace_abrir == 2){
			$externo = 1;
		}
		$this->_view->list_contenido_tipo = $this->getContenidotipo($this->_getSanitizedParam("padre"),$externo);
	}
	...
	public function manageAction()
	{
		...
		$this->_view->list_contenido_tipo = $this->getContenidotipo($padre,$externo);
	}
	...
	private function getContenidoseccion()
	{
		...
		$array['14'] = 'Inicio Delivery';
		$array['15'] = 'Inicio Express';
		return $array;
	}
     ...
     */
	private function getContenidotipo($idpadre=0,$externo)
	{
		$padre = $this->mainModel->getById($idpadre);
		$array = array();
		if($padre == 0 || $externo == 1 ){
			if($externo != 1){
    ...
    protected function getFilter()
    {
        ...
        if (Session::getInstance()->get($this->namefilter)!="") {
			$filters =(object)Session::getInstance()->get($this->namefilter);
			if($filters->contenido_seccion && $padre!='' ){
