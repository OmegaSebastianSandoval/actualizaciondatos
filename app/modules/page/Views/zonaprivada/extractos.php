...
if ($_GET['prueba_extractos'] == "1") {
    //print_r($_SESSION['extractos']);
    print_r($this->extractos);
}
...
foreach ($extractos as $key => $value) {
    /*
    $res = explode("/",$value);
    $ultimo_elemento = end($res);
    $aux = explode("_",$ultimo_elemento);
    $anio = substr($aux[0],0,4);
    $mes = substr($aux[0],4,2);
    */
    // /extractos/2026/02/20260106_013123555_001308225000012480.pdf
    $partes = explode('/', trim($value, '/'));
    $anio = $partes[count($partes) - 3]; // 2026
    $mes = $partes[count($partes) - 2]; // 02

    $array_meses[] = intval($mes);
    $array_anios[] = intval($anio);
}
...
<div class="container">
    <div class="row">
        <div class="col-12 titulo-contact text-center">
            <br>
            <h3 class="titulo_certificado">Extractos</h3>

            <div class="text-center bold mb-2">Apreciado socio, lo invitamos a diligenciar el siguiente formulario para
                descargar su extracto.<br></div>

            <div>______________</div>
        </div>
        <div class="col-12 text-center">
            <i class="fas fa-user"
                style="margin-right: 5px; color: #000000;"></i><?php echo $this->socio->socio_nombre; ?> <a
        ...
        <div class="col-12 text-center mt-4">
            ...
            <div class="row">
                        ...
                        <?php foreach ($extractos as $key => $value) { ?>
                            ...
                            <?php if (file_exists($value)) { ?>
                                <tr class="res<?php echo $anio . "-" . intval($mes); ?> resultados " style="display: none;">
                                    ...
                                    <td><a href="https://delivery.clubelnogal.com/page/zonaprivada/leerpdf/?filename=<?php echo $value; ?>&cert=ext&year=<?= $anio ?>&month=<?= $mes ?>"
                                            target="_blank"><button class="btn btn-cafe btn-sm">Descargar</button></a></td>
                                </tr>
                            <?php } ?>
                ...
                <div class="col-12 text-center d-none"><a href="/page/zonaprivada/certificados"
                        class="btn btn-sm btn-cafe" target="_self">Regresar</a></div>
                <div class="col-12 text-center"><a href="https://www.clubelnogal.com/oficina-virtual-club-el-nogal"
                        class="btn btn-sm btn-cafe" target="_top">Regresar</a></div>
...
<script>
    function filtrar () {
        ...
        if (typeof elements[0] !== "undefined" || mes == '' || mes == undefined) {
            //$("#sin_resultados").hide();
        } else {
            //$("#sin_resultados").show();
            $('#exampleModal_extractos').modal('show');
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body text-center">
                <h4 class="titulo_certificado">Su extracto aún no está disponible. Para cualquier consulta, le
                    agradecemos comunicarse con el área de Cartera en el teléfono 6013267700 Ext. 3970.</h4>
            </div>
